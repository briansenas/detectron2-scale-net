...
class CalibMapper(DatasetMapper):
    def __call__(self, dataset_dict):
        """
        Args:
            dataset_dict (dict): Metadata of one image, in Detectron2 Dataset format.
        ...
        # USER: Write your own image loading if it's not from a file
        image = utils.read_image(dataset_dict["file_name"], format=self.image_format)
        W_ori, H_ori, _ = image.shape
        res = Instances((W_ori, H_ori))
        res.proposal_boxes = Boxes(torch.from_numpy(np.asarray([(0, 0, W_ori, H_ori)])))
        dataset_dict["proposals"] = res
        dataset_dict["annotations"] = [
            dict(
                # Add one large bounding box as the whole image.
                bbox=np.asarray([(0, 0, W_ori, H_ori)]),
                bbox_mode=1,  # XYWH_MODE
                category_id=0,
            ),
        ]
        utils.check_image_size(dataset_dict, image)
...
class COCOScaleMapper(DatasetMapper):
    def __call__(self, dataset_dict):
        """
        Args:
            dataset_dict (dict): Metadata of one image, in Detectron2 Dataset format.
        ...
        # Scale data:
        dataset_dict["v0"] = -1
        dataset_dict["f_pixels_yannick"] = -1
        dataset_dict["yc_estCam"] = -1
        pitch = dataset_dict.get("pitch", None)
        vfov = dataset_dict.get("vfov", None)
        horizon = -1
        if pitch is not None and vfov is not None:
            ...
            dataset_dict["v0"] = v0
            dataset_dict["f_pixels_yannick"] = f_pixels_yannick
            y_person = 1.75
            vc = H / 2.0
            yc_list = []
        ...
        # USER: Remove if youV don't use pre-computed proposals.
        # Most users would not need this feature.
        if self.proposal_topk is not None:
            utils.transform_proposals(
                dataset_dict,
