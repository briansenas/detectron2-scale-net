...
def roll2soft_idx(angle):
    return angle_to_soft_idx(angle, min=np.min(roll_new_bins), max=np.max(roll_new_bins))
...
def convert_preds_to_angles(pred_vfov, pred_pitch, pred_roll, loss_type='kl', return_type='torch', legacy=False):
    if loss_type in ('kl', 'ce'):
        ...
        if not legacy:
            pred_roll = soft_idx_to_angle(get_softargmax(pred_roll), min=np.min(
                roll_new_bins), max=np.max(roll_new_bins))
        else:
            pred_roll = bins2roll(pred_roll)
